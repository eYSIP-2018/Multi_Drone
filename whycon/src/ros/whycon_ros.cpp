...
whycon::WhyConROS::WhyConROS(ros::NodeHandle& n) : is_tracking(false), should_reset(true), it(n)
{
  ...
  pixels_pub = n.advertise<geometry_msgs::Pose>("pixel_pose", 1);
...
{ 
  ...
  bool publish_pixels=  (pixels_pub.getNumSubscribers() != 0);
  ...
  for (int i = 0; i < system->targets; i++) {
    ...
    if (publish_images) {
      ...
      circle.draw(output_image, ostr.str(), cv::Vec3b(0,255,255));
			...
			//cv::circle(output_image, camera_model.project3dToPixel(cv::Point3d(coord)), 1, cv::Scalar(0,0,255), 1, CV_AA);
    }
